...
Tree.prototype.map = function (callback) {
  ...
  var map = function (currentTree, newTree, callback) {
    if(currentTree.children === 0){
      return;
    }
    for( var i = 0; i < currentTree.children.length; i++){
      var currentTreeChild = currentTree.children[i],
      treeChild = new Tree(callback(currentTreeChild.value));
      newTree.addChild(treeChild);
      
      map(currentTreeChild,treeChild , callback)
    }
    
  }
  var newRoot = new Tree(callback(this.value));
  map(this, newRoot, callback);
  return newRoot
};
